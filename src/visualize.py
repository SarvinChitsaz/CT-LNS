...
def visualize_ct_nodule(npy_folder, pred_thresh = 0.3):
    def overlay_mask(ct_slice, mask_slice, color = 'red', alpha = 0.4):
        ...
        if color == 'red':
            ...
            mask_rgb[..., 1] = mask_float
        elif color == 'blue':
            mask_rgb[..., 2] = mask_float
    ...
    if not volumes_with_nodule:
        print("No nodules found in the dataset!")
        return
    ...
    axes[2].set_title(f"Predicted Mask (thresh = {pred_thresh})")
    axes[2].axis('off')
